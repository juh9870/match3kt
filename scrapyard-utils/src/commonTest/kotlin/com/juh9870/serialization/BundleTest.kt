...
class BundleTest : FunSpec({
    context("Single values") {
        ...
        test("Int tests") {
            checkAll<Int>(1000000) {
                runSimpleTask(it, BundleWriter::write, BundleReader::readInt)
            }
        }

        test("Long tests") {
            checkAll<Long>(1000000) {
                runSimpleTask(it, BundleWriter::write, BundleReader::readLong)
            }
        }

        test("Float tests") {
            checkAll<Float>(1000000) {
                runSimpleTask(it, BundleWriter::write, BundleReader::readFloat)
            }
        }

        test("Double tests") {
            checkAll<Double>(1000000) {
                runSimpleTask(it, BundleWriter::write, BundleReader::readDouble)
            }
        }
    ...
    context("Arrays") {
        ...
        test("String arrays") {
            val gen = Arb.string()
            checkAll(Arb.int(0..5000).flatMap { length -> Arb.list(gen, length..length) }.map { it.toTypedArray() }) {
                runSimpleTask(it, BundleWriter::write, BundleReader::readString)
            }
        }
    }
